...
public class ShooterSubsytem extends SubsystemBase
{
    ...
    public ShooterSubsytem(XboxController controller)
    {
        ...
        shooterLeft = new CANSparkMax(DriveConstants.LEFT_SHOOTER_MOTOR_PORT, MotorType.kBrushed);
        shooterRight = new CANSparkMax(DriveConstants.RIGHT_SHOOTER_MOTOR_PORT, MotorType.kBrushed);
    ...
    {
        return this.runOnce(() ->
            robotPositonToApril = cameraSubsystem.getLastEstimatedRobotPose(false)
        );
    }

    public void buttonShoot()
    {
        if (robotPositonToApril.getX() < 5)
        {
            // DO SOMETHING
        }
        // Speaker button - 8, AMP Button - 7
        if(gcontroller.getRawButton(OIConstants.SPEAKER_BUTTON_PORT) || gcontroller.getRawButton(OIConstants.AMP_BUTTON_PORT))
        {
            feederMotor.set(0.3);
        }
        else 
